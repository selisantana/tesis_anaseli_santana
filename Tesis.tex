...
La planta consta de un biorreactor que contiene una resistencia para calentar la solución, una estructura que se soldó y modificó para adaptar correctamente los sensores tipo K y el tanque agitado con un mezclador, como se muestra en la figura. \ref{fig:soldado1}.
...
Para el caso de del tanque agitado, se mandó a soldar unos tubos que tienen el propósito de mantener los sensores en su lugar, y estos no se enreden con el mezclador. Son tres tubos que tienen un diámetro de 1 cm, estos están distribuidos dentro del reactor, como lo muestra la figura \ref{Reactor con 3 tubos.}, cada tubo corresponde a cada sensor dentro del reactor.
...
El tercer elemento que fue modificado del biorreactor fue el motor; el motor de la figura  anterior era de corriente alterna a 127 v, a 50-60 hz, con 142 rpm, lamentablemente no contaba con el torque necesario para mover los 6 l de agua con biomasa bagazo de caña como lo muestra la figura .
...
\end{figure}
 ...
 Por este motivo, se procedió a su re-diseño (en las imágenes  \ref{Medidas del mezclador vista lateral} se presentan las dimensiones del nuevo mezclador), y se encargó su construcción a un profesional especializado.
...
El mezclador y el reactor fueron replicados en el programa SolidWorks con el fin de tener una idea más clara de cómo se verá el mezclador dentro del reactor (ver Figura \ref{Mezclador_1} ).
...
El programa en general lee la señal mandada por los termopares K, posteriormente esta señal es convertida en un valor de temperatura, otro dato que es leído es la fecha y hora, para posteriormente ser guardado en una tira de datos dentro de un documento con formato CSV, este conteo y guardado se genera cada 5 segundos. Cada vez que los datos son guardados, serán mostrados en la pantalla. Finalmente, cuando interrumpimos manualmente el programa, finalizando el tiempo de pretratamiento, nos muestra una gráfica con los valores y podemos ver el archivo guardado. El archivo CSV puede visualizarse en Excel.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Convertidor: diseño y construcción}
\onehalfspacing
Se tiene una resistencia con una potencia de 600 w por lo que se busca que cuando entre la señal de entrada sea suficiente para utilizar la resistencia por lo que se optó por realizar un convertidor que ayudará a aumentar la potencia de entrada. El diseño del convertidor se puede observar en la Figura \ref{convertidor diagrama}.
\begin{figure}[H]
	\centering
...
\singlespacing
\subsection{ Análisis del comportamiento del Sistema ante variaciones en la entrada}
\onehalfspacing
...
\singlespacing
\subsection{Diseño de una Entrada con fines de identificación}
\onehalfspacing
