...
\singlespacing
\subsection{Sistema: Entrada de la Señal PRBS}
\onehalfspacing
...
\begin{figure}[H]
	\centering
	\includegraphics[width=6cm, height=7cm]{imagenes/Biorreactor.png}
...
Para el caso de del reactor se mando a soldar unos tubos que tienen el propósito de mantener los sensores en su lugar, y estos no se enreden con el mezclador. Son tres tubos que tienen un diámetro de 1 cm, estos están distribuidos dentro reactor, como lo muestra la figura \ref{Reactor con 3 tubos.}, cada tubo corresponde a cada sensor dentro del reactor.
% TODO: \usepackage{graphicx} required
\begin{figure}[H]
	\centering
	\includegraphics[width=7cm, height=5cm]{imagenes/reactor_tubos}
...
El tercer elemento que fue modificado del biorreactor fue el motor; el motor de la figura  anterior era de corriente alterna a 127 v, a 50-60 hz, con 142 rpm, lamentablemente no contaba con el torque necesario para mover los 6 l de agua con biomasa bagazo de caña.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=5cm, height=7cm, angle=90]{imagenes/motorreductor.png}
	\caption{Moto-reductor}
	\label{motorreductor}
\end{figure}
El motor cuenta con 142 rpm, trabaja a 24 V , trabaja con un torque nominal de 3.9 kN/m, se programo con ayuda de un rele para prender 10 segundos y que se apague otros 10 segundos, con el propósito de observar el comportamiento de la temperatura.
...
Dado que la temperatura máxima obtenida con la fuente del laboratorio no era suficiente, se optó por realizar un convertidor que ayudará a transformar de corriente alterna a corriente directa, permitiendo introducir una señal a la resistencia para controlar la temperatura y obtener una función de transferencia mediante identificación. El diseño del convertidor se puede observar en la Figura \ref{convertidor diagrama}.
\begin{figure}[H]
	\centering
...
\singlespacing
\section{Sistema: Entradas en la planta}
...
\end{figure}
 ...
 En la figura 	\ref{Entradaenplanta}, se muestra un esquema de cómo entra una señal b(t) que entra al convertidor en forma de voltaje, esta señal se potencia con ayuda de una señal externa (es decir un voltaje de CA) y posteriormente se mete a la resistencia. Finalmente, se obtiene una señal de salida [y(t)] en forma de temperatura. La temperatura dentro de esta es leída con ayuda de unos termopares que se encuentran dentro del reactor.

\singlespacing
\section{Sistema: Identificación del sistema y control}
\onehalfspacing
...
\singlespacing
\subsection{Pruebas para observar el comportamiento del sistema con PWM}
\onehalfspacing


Para obtener el sistema de la planta se debe observar el comportamiento de la temperatura dentro del biorreactor. Para ello, se realizaron distintas pruebas con PWM modificando el ciclo de trabajo que, con ayuda del convertidor, ingresa a la resistencia.
...
Se necesita un sistema que modele el comportamiento de la temperatura de la planta, por lo que se realizaron técnicas de identificación, como es el caso de la técnica OE (error de salida por sus siglas en ingles) donde se obtiene el sistema 
...
Con una respuesta del sistema exponencial convergente como se muestra en la imagen
